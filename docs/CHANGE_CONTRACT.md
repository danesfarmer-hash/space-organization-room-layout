...
The final drag persisted across reload on `38223069822d33d499da4897d209781ea087ee64`. Replay advanced through 37 pieces and completed, but the off-origin QA fixture's 3D SVG projected all parts outside the viewport even after Fit. The minimal projection correction subtracts the room center only for a room whose center is more than 100 inches from the origin on an axis; ordinary near-origin framing is preserved. This changes viewing coordinates only, never room or closet geometry. Verify the off-origin 3D rendering and replay after deployment.
