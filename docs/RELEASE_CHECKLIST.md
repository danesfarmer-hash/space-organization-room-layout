...
### 3D off-origin replay correction candidate — 2026-09-25 UTC

- The published `38223069822d33d499da4897d209781ea087ee64` passed the live card drag in cloud Chrome 1363 × 937: `Closet Run` moved below `Tafisa Finish`, persisted after reload, and was restored by dragging back. Replay reached `Ready · complete · 13 / 13 seconds · 37 pieces` with the project still `Saved · Revision 0`.
- **FAIL:** on the off-origin `QA Fixture / Filler Visual Check / Two Wall Test`, 3D was blank during and after replay. The SVG contained 210 constructed children but its first polygon started at x≈1432 beyond the 1000-wide viewBox; Fit did not change it. Reproduce by opening that fixture, switching to 3D, and pressing Fit or Reveal → Replay. This is a view projection defect, not missing construction pieces.
- The candidate centers only off-origin 3D axes whose room center exceeds 100 in from zero. The ordinary near-origin projection remains unchanged. Projection fixture and full `npm test` pass 44/44; embedded script syntax and diff check pass. Live visual check is required before marking the 3D and replay visibility gate PASS. No saved project geometry was edited.

### Final Pages check — 2026-09-25 UTC

- Published `main` `f1f7c7df8b37507e7571b52802bb4e9a2b1d8213`; active `index.html` served the corrected projection in cloud Chrome at 1363 × 936. The two-wall QA fixture's 3D view displayed both runs and shelves within the 1000 × 700 canvas; its first polygon moved from x≈1432 to x≈552. The prior blank 3D reproduction is resolved for this fixture. No customer project or QA revision was saved.
- Replay on that fixture showed a single first partition at 0/13 seconds and a complete 37-piece closet at 13/13 seconds, with `Saved · Revision 0` throughout. Pause and Replay reset were exercised on the preceding deployed build. The automated reveal fixture checks that every piece index is visited and the model is unchanged. This is a PASS for replay on the tested fixture; a frame-by-frame visual audit of every one of the 37 placements was not performed.
- End Panel marker eligibility, add/select/delete with exact width restoration, Front View button, typed toolbar rename with preserved SVG/action and Reset, and Properties drag/reload/restore passed live on the preceding publication. The projection commit changed only the `iso` view mapping, focused test, and documentation.
- Toe-kick contact at exactly 64 mm and shoe board/lip coordinates PASS automated geometric fixtures. Their live 3D/POV seam and appearance are NOT TESTED. End-panel fit across varied room shapes, full 3D collision/occlusion, and saved-model reload are NOT TESTED; do not infer those checks from this smoke run.
